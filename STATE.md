...
**Budget and order.** The pilot's per-epoch time sets how many of the 50 designed runs are affordable. Priority: random knock-in subsets (cheapest, and every downstream question waits on them) → knockout k = 10 → targeted knock-ins → knockout k = 50. Training hyperparameters stay pinned; only epochs for the small subsets earns an ablation.

## Next steps
- [ ] Pilot epoch time → run budget. (Claude)
- [ ] Encoder-space within-category test from the extracted features. (Claude)
- [ ] Build remaining subset dirs; submit as 1-GPU jobs; evaluate each checkpoint with `ood_distance_analysis.py` exactly as the 12 category models were. (Claude)
