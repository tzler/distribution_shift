...
**Budget and order.** One fine-tune is ~1.5–2 h on a B200 (~3 min/epoch), so all 50 runs are affordable (~90 GPU-hours, about a day in parallel) [[D12](REASONING.md#D12)]. Order: random knock-in subsets → knockout k = 10 → targeted and cross knock-ins → knockout k = 50. Training hyperparameters stay pinned; each condition gets its own similarity table so mining fills every epoch; evaluation is chained into each job.
