...
Environment: python 3.11 with numpy, scipy, pandas, matplotlib, pillow (scikit-learn for the
clustering scripts). On our cluster: `/vast/home/b/bonnen/.conda/envs/dev/bin/python`;
anywhere else, `pip install numpy scipy pandas matplotlib pillow scikit-learn`.
Run every script from the repository root.

## The short path (CPU, no cluster)

The tables in [`data/`](data/README.md) carry the results; these rebuild the main figures from them:

| figure | script | reads |
|---|---|---|
| the main result, 34 models × 11,634 trials | `scratch/analyze_round3_simple.py` | `data/all_categories_long.csv.gz` |
| relative, not absolute | `scratch/fig_absolute_vs_relative.py` | same |
| one trial, many models | `scratch/fig_one_trial_many_models.py` | same |
| the continuum plot | `scratch/fig_gradient.py` | same + descriptor banks |
| the 69-measure search | `scratch/distance_search_fast.py` | evaluation outputs + descriptor banks |
| the ladder | `scratch/analyze_ladder.py` | evaluation outputs |

Scripts that read `out/…` or `bank/…` need the lab share (those directories are not in git —
they are bulk and regenerable). Scripts that read `data/…` run anywhere.
Paths at the top of each script say which.

## Every cited figure

Copied from `evidence/README.md`, which is the authority and carries the commit each figure
was admitted at.

| figure | script | data | entry |
|---|---|---|---|
...
Fine-tuning and evaluation need the cluster and the collaborator's pipeline:
see [`experiments/README.md`](experiments/README.md). Nothing in `scratch/` trains a model —
the analysis and the experiments are deliberately separate.
