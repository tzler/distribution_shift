...
**What that buys inside the encoder's own space.** Computed in DINOv2-L features against the same training bank the models saw, the oddity-blind form does most of what we asked of it: the margin follows it within trial (r −0.48, stronger than anything we later got from geometry), and in the pooled row it leaves the pretrained margin flat (+0.03) — the control the manuscript's metric fails [D15]. What it does not do is behave on the on-category row: there the pretrained margin is predicted as strongly as the fine-tuned one (−0.24 vs −0.26), because in the encoder's space "far from the training set" and "looks unusual to this encoder" are nearly the same thing.

![the oddity-blind estimate inside the encoders' own spaces, against the same bank](../evidence/fig62_encoder_space.png)
