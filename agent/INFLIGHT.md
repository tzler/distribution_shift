...
## Pending a decision by the lead
- **A states/ snapshot for the current work (steps 8–10 in STATE.md: the anchor, absolute vs relative, the floor effect).** Entries D39–D44 are logged and the figures are admitted, but no `states/09-*.md` exists yet — the human-oversight default says the lead confirms a new snapshot. Once it exists, rebuild `../../lab-trace-workspace/distribution-shift-trace` with `demo-src/build_states.py` (it globs `states/0*.md`; extend the DATES list) and re-bundle the viewer.
- States 7 and 8 are still unreviewed by the lead.
