...
ESTS={'centroid':None,'cov_q02':None,'cov_q05':None,'cov_q10':None}
...
for nm,bank in banks.items():
    ...
    # radius for the coverage form: a quantile of all pairwise cosine distances among a random 3,000 bank objects of this descriptor
    ks=list(B); rng=np.random.default_rng(0); samp=np.stack([B[k] for k in rng.choice(ks,min(3000,len(ks)),replace=False)]); pd_=1-samp@samp.T; pd_=pd_[np.triu_indices(len(samp),1)]
    eps={'cov_q02':np.quantile(pd_,0.02),'cov_q05':np.quantile(pd_,0.05),'cov_q10':np.quantile(pd_,0.10)}
    for est in ESTS:
        dist=np.full((len(models),len(T)),np.nan)
        for mi,m in enumerate(models):
            S=np.stack([B[o] for o in train[m] if o in B]); c=S.mean(0); c/=np.linalg.norm(c)+1e-12
            for ti,t in enumerate(Tt):
                if t is None: continue
                if est=='centroid': dist[mi,ti]=(1-t@c).mean()
                else: dist[mi,ti]=-np.log1p(((1-t@S.T)<eps[est]).sum(1)).mean()   # more training nearby → more negative (so 'farther' is larger, like a distance)
...
R=pd.DataFrame(rows,columns=['descriptor','estimator','missing','all_fit','all_test','own_fit','own_test','oth_fit','oth_test']); R.to_csv(f'{G}/out/distance_search_set.csv',index=False)
